...
#define PROID		"linxi_smarthome"

#define AUTH_INFO	"linxi_smarthome_haokan"

#define DEVID		"linxi_smarthome_000000001"
...
void OneNet_RevPro(unsigned char *cmd)
{
    ...
    cJSON *json,*json_value,*json_key;
	...
	switch(type)
	{
		...
		case MQTT_PKT_PUBLISH:														//接收的Publish消息
			...
			if(result == 0)
			{
                ...
                if(!json)UsartPrintf(USART_DEBUG, "ERROR");
                else
                {
                    json_key =  cJSON_GetObjectItem(json,"target");
                    json_value =cJSON_GetObjectItem(json,"value");
                
                    if(strcmp("LED",json_key->valuestring) == 0)
                    {   
                        
                        if(json_value ->valueint == 1) //json——value>0且为整形则打开led
                        {
                            LED0 = 0; //打开LED0
                        }
                        else
                    ...
                    if(strcmp("beep",json_key->valuestring) == 0)
                    {
                         if(json_value ->valueint == 1) //json——value>0且为整形则打开led
                        {
                            LED1 = 0; //打开LED0
